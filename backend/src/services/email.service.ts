...
export class EmailService {
  async sendPasswordResetEmail(email: string, token: string) {
    ...
    if (!apiKey) {
      throw new AppError("BREVO_API_KEY not configured", 500);
    }

    if (!senderEmail) {
      throw new AppError("BREVO_SENDER_EMAIL not configured", 500);
    }
    ...
    if (!response.ok) {
      ...
      throw new AppError(
        errorMessage || "Brevo password reset email failed",
        response.status || 502
      );
    }
    ...
    if (process.env.NODE_ENV !== "production") {
      const messageId =
        result && "messageId" in result ? result.messageId : "no-id";
